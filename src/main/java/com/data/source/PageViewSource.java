 ...
 *
 */
public class PageViewSource implements SourceFunction<PageViewRecord> {
	 ...
	 */
	public PageViewSource(String dataFilePath) {
	 ...
	 */
	public PageViewSource(String dataFilePath, int maxEventDelaySecs) {
	 ...
	 */
	public PageViewSource(String dataFilePath, float servingSpeedFactor) {
	 ...
	 * @param servingSpeedFactor The serving speed factor by which the logical serving time is adjusted.
	 */
	public PageViewSource(String dataFilePath, int maxEventDelaySecs, float servingSpeedFactor) {
	...
	@Override
	public void run( SourceContext<PageViewRecord> sourceContext) throws Exception {
	...
	private void generateOrderedStream( SourceContext<PageViewRecord> sourceContext) throws Exception {
		...
		if ( reader.ready() && (line = reader.readLine()) != null) {
			// read first ride
			PageViewRecord pageViewRecord = PageViewRecord.fromString(line);
			// extract starting timestamp
			dataStartTime = pageViewRecord.getTimestamp();
			...
			// emit first ride
			sourceContext.collectWithTimestamp(pageViewRecord, pageViewRecord.getTimestamp());
		} else {
			return;
		}
		...
		while (reader.ready() && (line = reader.readLine()) != null) {
			...
			PageViewRecord pageViewRecord = PageViewRecord.fromString(line);
			long eventTime = pageViewRecord.getTimestamp();
	...
	private void generateUnorderedStream( SourceContext<PageViewRecord> sourceContext) throws Exception {
		...
		if (reader.ready() && (line = reader.readLine()) != null) {
			// read first ride
			pageViewRecord = PageViewRecord.fromString( line );
			// extract starting timestamp
			dataStartTime = pageViewRecord.getTimestamp();
			// get delayed time
			long delayedEventTime = dataStartTime + getNormalDelayMsecs(rand);
			emitSchedule.add(new Tuple2<Long, Object>( delayedEventTime, pageViewRecord));
			// schedule next watermark
		...
		while (emitSchedule.size() > 0 || reader.ready()) {
			...
			long rideEventTime = pageViewRecord != null ? pageViewRecord.getTimestamp() : -1;
			while(
					pageViewRecord != null && ( // while there is a ride AND
					...
					)
			{
				...
				if (reader.ready() && (line = reader.readLine()) != null) {
					...
					pageViewRecord = PageViewRecord.fromString( line );
					rideEventTime = pageViewRecord.getTimestamp();
				}
				else {
					pageViewRecord = null;
					rideEventTime = -1;
			...
			if(head.f1 instanceof TaxiRide) {
				PageViewRecord emitRide = (PageViewRecord)head.f1;
				// emit ride
				sourceContext.collectWithTimestamp(emitRide, emitRide.getTimestamp());
			}
